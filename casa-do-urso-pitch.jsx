...
function Sidebar({ active }) {
  return (
    <div style={{ width: 148, background: "#FAF7F3", borderRight: `1px solid ${C.grayLight}`, padding: "12px 0", flexShrink: 0 }}>
...
function ProblemMockup() {
  const actors = [
    ...
    { label: "E-mail",    Icon: FileText,       color: C.blue,    pos: { top: "10%", left: "65%" } },
    { label: "Planilha",  Icon: BarChart3,      color: C.teal,    pos: { top: "52%", left: "80%" } },
    { label: "Papel",     Icon: ClipboardList,  color: C.gray,    pos: { top: "75%", left: "58%" } },
    { label: "Ligação",   Icon: Bell,           color: C.purple,  pos: { top: "72%", left: "16%" } },
    { label: "Reunião",   Icon: Users,          color: C.gray,    pos: { top: "42%", left: "6%"  } },
  ];
  return (
    <MockupShell title="Estado atual do cuidado">
      <div style={{ padding: 20, height: 300, position: "relative" }}>
        {/* SVG dashed connectors */}
        <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none" }}>
          {[["14%","18%"],["12%","65%"],["54%","80%"],["77%","60%"],["74%","18%"],["44%","8%"]].map(([cy, cx], i) => (
            <line key={i} x1="46%" y1="48%" x2={cx} y2={cy}
              stroke={C.red} strokeWidth="1" strokeDasharray="4,4" opacity="0.3" />
          ))}
        </svg>
        {/* Center child */}
        <div style={{
          position: "absolute", top: "44%", left: "44%", transform: "translate(-50%,-50%)",
          width: 58, height: 58, borderRadius: "50%",
          background: C.orangeLight, border: `2px solid ${C.orange}`,
          display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
          zIndex: 10,
        }}>
          <Baby size={20} color={C.orange} strokeWidth={1.8} />
          <span style={{ fontSize: 8, fontWeight: 700, color: C.orange }}>Criança</span>
        </div>
        {/* Actor nodes */}
        {actors.map((a, i) => {
          const Icon = a.Icon;
          return (
            <div key={i} style={{
              position: "absolute", ...a.pos, transform: "translate(-50%,-50%)",
              display: "flex", flexDirection: "column", alignItems: "center", gap: 3, zIndex: 5,
            }}>
              <div style={{
                width: 38, height: 38, borderRadius: 10,
                background: `${a.color}15`, border: `1.5px dashed ${a.color}50`,
                display: "flex", alignItems: "center", justifyContent: "center",
              }}>
                <Icon size={17} color={a.color} strokeWidth={1.5} />
              </div>
              <span style={{ fontSize: 8, color: C.gray, fontWeight: 500 }}>{a.label}</span>
            </div>
          );
        })}
        {/* Warning bar */}
        <div style={{
          ...
          background: "#FFF5F5", borderTop: `1px solid ${C.red}25`,
          padding: "8px 14px", display: "flex", alignItems: "center", gap: 6,
        }}>
          <X size={12} color={C.red} strokeWidth={2.5} />
          <span style={{ fontSize: 10, color: C.red, fontWeight: 600 }}>Nenhum dado compartilhado de forma estruturada</span>
        </div>
      </div>
    </MockupShell>
  );
...
function SolutionMockup() {
  const spokes = [
    ...
    { label: "Escola",     sub: "educadores",  color: C.teal,   Icon: GraduationCap, angle: 0  },
    { label: "Família",    sub: "pais",        color: C.blue,   Icon: Users,         angle: 120 },
    { label: "Supervisor", sub: "coord.",      color: C.purple, Icon: Stethoscope,   angle: 240 },
  ];
  const R = 108;
  const cx = 160, cy = 148;
  return (
    <MockupShell title="Casa do Urso Platform Ecossistema">
      <div style={{ padding: 20, height: 310, position: "relative" }}>
        <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none" }}>
          ...
          <circle cx={cx} cy={cy} r={R * 0.62} fill="none" stroke={`${C.orange}08`} strokeWidth="1" />
          {spokes.map((s, i) => {
            const rad = s.angle * Math.PI / 180;
            const x = cx + R * Math.cos(rad), y = cy + R * Math.sin(rad);
            return <line key={i} x1={cx} y1={cy} x2={x} y2={y} stroke={s.color} strokeWidth="1.5" opacity="0.5" />;
          })}
        </svg>
        {/* Center node */}
        <div style={{
          position: "absolute", top: cy - 38, left: cx - 38,
          width: 76, height: 76, borderRadius: "50%",
          background: C.orangeLight, border: `2px solid ${C.orange}`,
          display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 2,
        }}>
          <Baby size={24} color={C.orange} strokeWidth={1.8} />
          <span style={{ fontSize: 8, fontWeight: 700, color: C.orange }}>Criança</span>
        </div>
        {/* Spoke nodes */}
        {spokes.map((s, i) => {
          ...
          return (
            <div key={i} style={{
              position: "absolute", top: y - 30, left: x - 38,
              width: 76, display: "flex", flexDirection: "column", alignItems: "center", gap: 3,
            }}>
              <div style={{
              ...
              <span style={{ fontSize: 8, color: C.gray }}>{s.sub}</span>
            </div>
          );
        })}
        {/* Feature pills */}
        <div style={{
          position: "absolute", bottom: 0, left: 0, right: 0,
          background: C.orangeLight, borderTop: `1px solid ${C.orangeMid}`,
          padding: "8px 14px", display: "flex", gap: 6, flexWrap: "wrap",
        }}>
          {["Sessões","Relatórios","PEI","Mensagens","Escola","Mídia"].map(f => (
            <span key={f} style={{ fontSize: 9, background: C.white, color: C.orange, border: `1px solid ${C.orangeMid}`, padding: "3px 8px", borderRadius: 20, fontWeight: 600 }}>{f}</span>
          ))}
        </div>
      </div>
    </MockupShell>
...
function TractionMockup() {
  const milestones = [
    ...
    { label: "Validação com terapeutas e famílias",  done: true,   active: false },
    { label: "MVP da plataforma em construção",      done: false,  active: true  },
    { label: "Primeiros clientes piloto",            done: false,  active: false },
    { label: "Expansão nacional · SaaS",             done: false,  active: false },
  ];
      ...
      <div style={{ display: "flex", height: 310 }}>
        ...
        <div style={{ flex: 1, padding: 14, overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            {/* Revenue bars */}
            <div style={{ flex: 1, background: C.cream, borderRadius: 10, padding: 12 }}>
              <div style={{ fontSize: 10, fontWeight: 600, color: C.dark, marginBottom: 10 }}>Receita mensal (R$ mil)</div>
              <div style={{ display: "flex", alignItems: "flex-end", gap: 6, height: 68 }}>
                {bars.map((v, i) => (
                  <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
                    <div style={{
                      width: "100%", borderRadius: "3px 3px 0 0",
                      background: i === bars.length - 1 ? C.orange : C.orangeMid,
                      height: `${(v / 30) * 60}px`,
                    }} />
                    <span style={{ fontSize: 8, color: C.gray }}>M{i + 1}</span>
                  </div>
                ))}
              </div>
            </div>
            {/* Roadmap */}
            <div style={{ flex: 1.3, background: C.cream, borderRadius: 10, padding: 12 }}>
              <div style={{ fontSize: 10, fontWeight: 600, color: C.dark, marginBottom: 10 }}>Roadmap</div>
              {milestones.map((m, i) => (
                <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start", marginBottom: 7 }}>
                  <div style={{
                    width: 11, height: 11, borderRadius: "50%", flexShrink: 0, marginTop: 1,
                    background: m.done ? C.orange : "transparent",
                    border: `2px solid ${m.done ? C.orange : m.active ? C.orange : C.grayLight}`,
                    outline: m.active ? `2px solid ${C.orangeMid}` : "none",
                    outlineOffset: 2,
                  }} />
                  <span style={{ fontSize: 9, color: m.done ? C.dark : m.active ? C.orange : C.gray, fontWeight: m.active ? 700 : 400, lineHeight: 1.4 }}>{m.label}</span>
                </div>
              ))}
            </div>
          </div>
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", color: C.dark }}>
      ...
      <nav style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontFamily: "'DM Serif Display', serif", fontSize: 17, color: C.dark }}>Casa do Urso</span>
